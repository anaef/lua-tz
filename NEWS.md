...
saving time to standard time, and an 'hour' thus repeats on the same calendar day.
...
> [!IMPORTANT]
> Together, the two changes to `lua-tz` mean that it is often no longer necessary to clear the
> `off` and `isdst` fields when converting calendar date and time among time zones, thus
> eliminating a source of error. However, if your code (unintuitively) relies on the `off` field
> taking precedence over an explicitly provided time zone, it must be updated.
